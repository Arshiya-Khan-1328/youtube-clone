...
const VideoDetail = () => {
  ...
  useEffect(() => {
    ...
    .then((data)=> setplaybackDetail(data.items[0]))

    fetchFromAPI(`search?part=snippet&relatedToVideoId=${id}&type=video`)
    .then((data)=> setclips(data.items))
  }, [id]);

  if(!playbackDetail?.snippet) return 'Loading...';


  const {snippet: {title, channelId, channelTitle}, statistics:{viewCount, likeCount}} = playbackDetail;
  ...
  return (
    ...
    <Videos videos={clips} direction= "column"/>
      </Box>             
      </Stack>
    </Box>
  )
